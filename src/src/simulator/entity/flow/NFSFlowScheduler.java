...
public abstract class NFSFlowScheduler extends Entity{
	...
	public class NFSFlowRateComparator implements Comparator<NFSFlow> {
		...
		@Override
		public int compare(NFSFlow flow1, NFSFlow flow2) {
			double datarate1 = flow1.status.equals(NFSFlowStatus.NEWSTARTED) || flow1.status.equals(NFSFlowStatus.ADJUSTING) ? 
					flow1.expectedrate : flow1.datarate;
			double datarate2 = flow2.status.equals(NFSFlowStatus.NEWSTARTED) || flow2.status.equals(NFSFlowStatus.ADJUSTING)? 
					flow2.expectedrate : flow2.datarate;
